...
SOLUBILITY = 1.34e-9  # Растворимость кислорода в воде/гидрогеле при 37°C (моль/(см^3 * mmHg))
V_MAX = 1.2e-16        # Максимальная скорость потребления кислорода одной клеткой (моль/(клетка * с))
K_M = 0.5             # Константа полунасыщения (Michaelis constant) для выживания клеток (mmHg)
K_M_INSULIN = 5.0     # Константа полунасыщения для активной секреции инсулина (mmHg)

# Пресеты гидрогелей и их коэффициенты диффузии кислорода (см^2 / с)
HYDROGELS = {
    "water": {
        "name": "Чистая вода (контроль)",
        "D": 3.0e-5
    },
    "alginate_1%": {
        "name": "1% Альгинат натрия",
        "D": 2.1e-5
    },
    "alginate_2%": {
        "name": "2% Альгинат натрия (стандарт)",
        "D": 1.5e-5
    }
}
...
IMPLANTATION_SITES = {
    "arterial": {
        "name": "Артериальное русло (прямая перфузия)",
        "pO2": 95.0,
        "description": "Идеальные условия, высокая оксигенация (например, сосудистый порт)."
    },
    "venous": {
        "name": "Венозная сеть (или воротная вена)",
        "pO2": 40.0,
        "description": "Умеренная оксигенация, характерная для внутренних органов."
    },
    "subcutaneous": {
        "name": "Подкожная клетчатка (типичное место введения)",
        "pO2": 30.0,
        "description": "Низкое давление кислорода. Самое доступное, но сложное место для приживления."
    },
    "extreme_hypoxia": {
        "name": "Зона фиброза / Выраженная гипоксия",
        "pO2": 10.0,
        "description": "Худший сценарий. Капсула обросла фиброзной тканью, доступ кислорода сильно ограничен."
    }
}
